...
bool BFdj::chargerDonneesFdjeux(stGameConf *pGame, QString destTable)
{
 ...
 ///   {"sloto.csv",fId++, {false,2,1,1,&resLoto[0]} },
    ...
    static  stZnDef zn_loto_5[] = {
                                    { 4, 5, 1, 49, 5 },
                                    { 9, 1, 1, 10, 1 }
                                };

    static  stZnDef zn_loto_5b[] = {
        { 32, 5, 1, 49, 5 }
    };
    ...
    stFdjData loto[] = {
     ...
     { "loto.csv", fId++, { true, 3, 2, 1, &res_loto_3 } },
        ...
        { "loto_201911.csv", fId++, { true, 2, 1, 2, res_loto_5 } },
...
bool BFdj::LireLesTirages(stGameConf *pGame, stFdjData *def, QString tblName)
{
  ...
  int nbResuLgn = def->param.nbResu;
        ...
        for(int un_resu=0;(un_resu<nbResuLgn) && b_retVal; un_resu++)
        {
            ...
            // Nombre de zones composant ce resultat
            ...
            for(int zone=0;zone< max_zone;zone++)
            {
                ...
                {
#if 0
                    QString raw = list1.at(curZn->pos + ElmZone);
                    bool okNum = false;
                    int  val1  = raw.toInt(&okNum);

                    bool isMissing = (!okNum) || (val1 == 0);  // 0 ou non convertible => manquant
                    msgColZn += pGame->names[zone].abv + QString::number(ElmZone+1);

                    if (isMissing) {
                        msgValZn += "NULL"; // SANS quotes
                    } else if (val1 >= minValZone && val1 <= maxValZone) {
                        msgValZn += QString::number(val1);
                    } else {
                        // soit on log + NULL, soit on skip, mais on ne fait pas return false
                        // msgValZn += "NULL";
                        // ou si tu préfères: continue;
                        msgValZn += "NULL";
                    }
