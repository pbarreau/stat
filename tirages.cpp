...
tirages::tirages(NE_FDJ::E_typeJeux jeu)
{
    ...
    for(zone = 0; zone < conf.nb_zone; zone++)
    {
        ...
        value.valBoules[zone] =  new int [conf.limites[zone].len];
        for(j=0;j<conf.limites[zone].len;j++)
        {
...
QString tirages::qs_zColBaseName(int zone)
{
    ...
    int items = conf.limites[zone].len;

    for(elem = 0; elem < items;elem++)
    {
        msg1 = msg1 + conf.TT_Zn[zone].abv + QString::number(elem +1) +",";
    }

#if 0
    for(elem = 0; elem < (this->conf.limites[zone].len);elem++)
    {
        msg1 = msg1 + this->conf.TT_Zn[zone].abv + QString::number(elem +1) +",";
    }
#endif
...
QString tirages::s_LibColAnalyse(stTiragesDef *pRef)
{
    ...
    for(int zone=0;zone<ref.nb_zone ;zone++)
    {
        ...
        {
            //int val = j%10;
            msg2 = msg2 + ref.TT_Zn[zone].abv + "d"+QString::number(j)+",";
        }
    }
...
QString tirages::s_LibColBase(stTiragesDef *ref)
{
    ...
    {
        elem = zn_conf[zone].len;
        for(j=0;j<elem;j++)
        {
            msg1 = msg1 + tab[zone].abv+QString::number(j+1) +",";
        }
    }
    ...
    for(zone=0;zone<nbZn;zone++)
    {
        msg1 = msg1 + tab[zone].abv + CL_PAIR + ",";
    }

    // Colonnes pour appartenance sous groupe
    for(zone=0;zone<nbZn;zone++)
    {
        msg1 = msg1 + tab[zone].abv + CL_SGRP +",";
    }
...
int tirages::RechercheNbBoulesLimite(int zone, int min, int max)
{
    int i;
    int ret = 0;
    for(i=0;i<(conf.limites[zone].len);i++)
    {
