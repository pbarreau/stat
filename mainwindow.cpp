...
void MainWindow::EtudierJeu(etFdj curGame, bool use_odb, bool fdj_new, bool upl_fdj)
{
    ...
    //BXmlFdj my_xml = BXmlFdj();
    //return;

    // 1) Paramètre sur la pile (pas de fuite)
    stFdj prm{};
    prm.typeJeu = curGame;
    prm.use_odb = use_odb;
    prm.fdj_new = fdj_new;
    ...
    // 2) BFdj parenté à la MainWindow => destruction auto
    //    (ajoutez un ctor BFdj(stFdj*, QObject* parent = nullptr))
    auto *charge = new BFdj(&prm);
    charge->setParent(this);

    // 3) Optionnel mais recommandé : ajouter dans BFdj un indicateur de succès
    if (!charge->isReady()) {
        QMessageBox::warning(this, tr("FDJ"), tr("Initialisation FDJ incomplète."));
        return;
    }
    ...
    if (!curConf) {
        QMessageBox::warning(this, tr("Erreur"), tr("Configuration de jeu indisponible."));
        return;
    }

    qInfo() << "[EtudierJeu] game=" << int(curGame)
            << "use_odb=" << use_odb
            << "fdj_new=" << fdj_new
            << "upl_fdj=" << upl_fdj;

    qInfo() << "[BFdj] ready=" << charge->isReady();
    qInfo() << "[BFdj] curConf=" << static_cast<void*>(curConf);
    ...
    // Parent visuel = un conteneur (onglet, splitter, zone centrale…)
    auto *lst_tirages = new BTirFdj(curConf, charge, eTirFdj, this);
    ...
    auto *ana_tirages = new BTirAna(curConf, this);  // parent = MainWindow (ou ton conteneur)
    ...
    if(ana_tirages->self() == nullptr){
        ...
        // Déclenche l’analyse après que l’UI ait repris la main
        QTimer::singleShot(0, ana_tirages, [ana_tirages]{
            ana_tirages->startAsync(); // Nouvelle API non bloquante (voir ci-dessous)
        });
    }
