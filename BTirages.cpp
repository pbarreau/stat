...
QWidget *BTirages::showFdj(BTirAna *ana_tirages)
{
 ...
 lay_visual->show();
    ...
    /// Resultats base complete
    connect(og_Items,SIGNAL(tabBarClicked(int)),this,SLOT(BSlot_Tir_flt(int)));
...
void BTirages::BSlot_Tir_flt(int index)
{
    ...
    BTirAna ** tmp = nullptr;
    BTirAna * tmp_ana = nullptr;
    QString msg ="";
    QString box_title = "";

    if(from != og_Items){
        tmp = ana_TirFlt->at(id_AnaOnglet);
        tmp_ana = qobject_cast<BTirAna *>(tmp[index]);

        /// Pour rappeller la selection choisie
        //emit BSig_Show_Flt(save_sel);

        box_title = og_AnaSel->tabText(id_AnaOnglet)+" ("+from->tabText(index)+"). ";
        msg = tmp_ana->getSql();
        updateTbv(box_title, msg);
    }
    else{
        msg = getTiragesList(gme_cnf, game_lab);
#if 0
        tmp_ana = ana_TirLst;
        msg = tmp_ana->getSql().simplified();
        if(msg.size() == 0){
            msg = getTiragesList(gme_cnf, game_lab);
        }
#endif
...
void BTirages::BSlot_Result_Tir(const int index)
{
    ...
    if(child_1.size()){
        ...
        // Emission du signal on a clique  sur l'onglet
