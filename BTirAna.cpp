...
#if 0
BTirAna::BTirAna(stGameConf *pGame, QWidget *parent) : QWidget(parent), m_game(pGame)
{
    addr = nullptr;
...
#else
// BTirAna.cpp (extrait)
BTirAna::BTirAna(stGameConf *pGame, QWidget *parent)
    : QWidget(parent), m_game(pGame)
{
    addr = this;
    typeAnalyse = eTirNotSet;
    ...
    m_db  = QSqlDatabase::database(pGame->db_ref->cnx);

    // Optionnel : UI squelette
    auto *lay = new QVBoxLayout(this);
    auto *lbl = new QLabel(tr("Préparation de l'analyse…"));
    lay->addWidget(lbl);
    setLayout(lay);
}
#endif
...
void BTirAna::startAsync() {
    ...
    auto future = QtConcurrent::run([baseCnxName, base, dbFile, tblName](){
        ...
        QSqlDatabase::removeDatabase(workerCnx);
    ...
    connect(watcher, &QFutureWatcherBase::finished, this, [=](){
        ...
        watcher->deleteLater();
        emit sigFinished(res.first, res.second);
    });
    watcher->setFuture(future);

    // Progres: si tu peux, émettre depuis le thread via signaux intermédiaires (Qt::QueuedConnection)
}
