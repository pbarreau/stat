 ...
 if(b_retVal){
  ...
  if((b_retVal = query.exec(st_query))){
   ...
   if(version < "3.25"){
    ...
    if(!(b_retVal=AuthoriseChargementExtension(mabase))){
...
bool BFdj::AuthoriseChargementExtension(QString dbPath)
{
 bool b_retVal = true;
 QSqlQuery query(fdj_db);
 QString msg = "";
 ...
 int thread_value = sqlite3_threadsafe();
  ...
  if (handle != 0) { // check that it is not NULL
   ...
   // b_retVal = query.exec(msg);
            QString extensionPath = "libStatPgm-extension-functions-x86_64.dll";
            b_retVal = SQLiteExtensionLoader::loadExtension(dbPath, extensionPath);
#ifndef QT_NO_DEBUG
