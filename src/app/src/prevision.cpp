...
bool MainWindow::NEW_FaireBilan(int idTirage, QString stTblRef,int zone, stTiragesDef *pConf, int nbRang)
{
 ...
 for(int i = 1; (i<=nbRang) && status == true;i++)
 {
  msg1 = "insert into tmpBilan (d, r, b, t, s) select D, R, B, T,Smax from "
         ...
         "("
         "select d, id, b1k as blgn, b1r as sortie  from "
         +nomTable
         +" where id =" +QString::number(i)
         + " "
           "union all "
           "select d, id, b2k as blgn, b2r as sortie  from "
         +nomTable+" where id ="
         +QString::number(i)
         + " "
           "union all "
           "select d, id, b3k as blgn, b3r as sortie   from "
         +nomTable
         +" where id ="
         +QString::number(i)
         + " "
           "union all "
           "select d, id, b4k as blgn, b4r as sortie   from "
         +nomTable
         +" where id =" +QString::number(i)
         + " "
           "union all "
           "select d, id,b5k as blgn, b5r as sortie   from "
         +nomTable
         +" where id ="
         +QString::number(i)
         + " "
