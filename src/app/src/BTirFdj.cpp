...
BTirFdj::BTirFdj(const stGameConf *pGame, BFdj * worker, etTir gme_tir, QWidget *parent): BTirages(pGame, worker, gme_tir, parent)
{
    db_fdj = db_tir;

    connect(this, SIGNAL(BSigMyQueryExec(const QString,const QString, BView*)),
            worker, SLOT(BSlotMyQueryResults(const QString,const QString, BView*)));
...
QWidget *BTirFdj::tbForBaseRef(const stGameConf *pGame)
{
    ...
    //// A analyser pour exec en parrallele
    //emit BSigMyQueryExec(msg,tbl_tirages,qtv_tmp);
...
{
    BAction_1 *cmd_1 = qobject_cast<BAction_1 *>(sender());

    BView * ptr_qtv = nullptr;
    BFpmFdj * fpm_tmp = nullptr;
    QSqlQueryModel *sqm_tmp = nullptr;
    QSqlQueryModel *sqm_tmp_2 = nullptr;
    QString start_1 = "";
    QString start_2 = "";

    int col = index.column();
    int row = index.row();
    int c_0 = index.model()->index(index.row(),0).data().toInt();
    int c_1 = index.sibling(row,0).data().toInt();

    ptr_qtv = cmd_1->getView();
    fpm_tmp = qobject_cast<BFpmFdj *>(ptr_qtv->model());
    sqm_tmp = qobject_cast<QSqlQueryModel *> (fpm_tmp->sourceModel());
    start_1 = sqm_tmp->query().executedQuery();
