#include "BMultiSelectComboBox.h"
#include <QLineEdit>
#include <QCheckBox>
#include <QEvent>
...
BMultiSelectComboBox::BMultiSelectComboBox(QWidget* aParent) :
    QComboBox(aParent),
    ...
    mSearchBar(new QLineEdit(this))
{
    ...
    connect(mSearchBar, &QLineEdit::textChanged, this, &BMultiSelectComboBox::onSearch);
    connect(this, static_cast<void (QComboBox::*)(int)>(&QComboBox::activated), this, &BMultiSelectComboBox::itemClicked);
}

void BMultiSelectComboBox::hidePopup()
{
...
void BMultiSelectComboBox::stateChanged(int aState)
{
    Q_UNUSED(aState);
...
void BMultiSelectComboBox::addItem(const QString& aText, const QVariant& aUserData)
{
    Q_UNUSED(aUserData);
    ...
    connect(checkBox, &QCheckBox::stateChanged, this, &BMultiSelectComboBox::stateChanged);
}

QStringList BMultiSelectComboBox::currentText()
{
...
void BMultiSelectComboBox::addItems(const QStringList& aTexts)
{
...
int BMultiSelectComboBox::count() const
{
...
void BMultiSelectComboBox::onSearch(const QString& aSearchString)
{
...
void BMultiSelectComboBox::itemClicked(int aIndex)
{
...
void BMultiSelectComboBox::SetSearchBarPlaceHolderText(const QString& aPlaceHolderText)
{
    mSearchBar->setPlaceholderText(aPlaceHolderText);
}

void BMultiSelectComboBox::SetPlaceHolderText(const QString& aPlaceHolderText)
{
    mLineEdit->setPlaceholderText(aPlaceHolderText);
}

void BMultiSelectComboBox::clear()
{
    mListWidget->clear();
    QListWidgetItem* curItem = new QListWidgetItem(mListWidget);
    mSearchBar = new QLineEdit(this);
...
void BMultiSelectComboBox::wheelEvent(QWheelEvent* aWheelEvent)
{
    // Do not handle the wheel event
    Q_UNUSED(aWheelEvent);
}

bool BMultiSelectComboBox::eventFilter(QObject* aObject, QEvent* aEvent)
{
...
void BMultiSelectComboBox::keyPressEvent(QKeyEvent* aEvent)
{
    // Do not handle key event
    Q_UNUSED(aEvent);
}

void BMultiSelectComboBox::setCurrentText(const QString& aText)
{
    Q_UNUSED(aText);
}

void BMultiSelectComboBox::setCurrentText(const QStringList& aText)
{
    int count = mListWidget->count();
...
void BMultiSelectComboBox::ResetSelection()
{
    int count = mListWidget->count();
