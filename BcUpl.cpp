...
void BcUpl::BSlot_UpdateTbv(const stParam_tsk *tsk_param)
{
#if 0
 ...
 ///
 QMap<QString, stUplBViewPos>::const_iterator it;
 it = tsk_param->lst_view->find(t_use);
 if(it != tsk_param->lst_view->end()){
  BView *qtv_tmp = it->view;
#if 0
  //it->view->reset();
...
QWidget *BcUpl::Mk1_MainUplet(stParam_tsk *tsk_param)
{
 ...
 BAnimateCell *ani = nullptr; //tbv_Anim[l_id-1][z_id][g_id-C_MIN_UPL];
 tsk_param->a_tbv = ani;
